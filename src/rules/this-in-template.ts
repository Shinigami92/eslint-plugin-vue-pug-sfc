...
export default {
  ...
  create(context) {
    return processRule(context, () => {
      const option: 'always' | 'never' =
        context.options[0] !== 'always' ? 'never' : 'always';
      ...
      function handle(
        ...
        if (
          ...
          if (option === 'never') {
            ...
            const textSlice: string = context
              .getSourceCode()
            ...
            const columnEnd: number =
              columnStart + 'this'.length + (withOptionalChaining ? 1 : 0);
            ...
            context.report({
              ...
              fix(fixer) {
                ...
                const removeTo: number =
                  removeFrom + 'this.'.length + (withOptionalChaining ? 1 : 0);
